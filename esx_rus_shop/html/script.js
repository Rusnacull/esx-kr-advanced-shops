...
$(function() {
		window.addEventListener('message', function(event) {
			...
			$('#wrapper').append(
				` <h6 class = "h4" style = "right: 47.125%; position: absolute;" bottom = ${id - 5}>Magazin din Orasul Iasi</h6>`
			)
			...
			if (event.data.owner == true) {
				...
				$('#wrapper').append(
					`
					<button class = "button" id = "bossactions" style = "position: absolute; right: 15px; top: 5px;">Meniu Patron</button>
					`
				);
			}
			...
			$('.image').on('click', function () {
				...
				$.post('http://esx_rus_shop/putcart', JSON.stringify({item: item, price : price, label : label, count : count, id : id}), function( cb ) {
			...
			$("body").on("click", "#refreshcart", function() {
				$.post('http://esx_rus_shop/escape', JSON.stringify({}));
				location.reload(true);
				$('#wrapper').hide("fast");
				$('#payment').hide("fast");
				$('#cart').hide("fast");
				$.post('http://esx_rus_shop/refresh', JSON.stringify({}));
			});
			...
			$("body").on("click", "#buybutton", function() {
				...
				for (i = 0; i < value.length; i++) {
					...
					if (parseInt(count) >= parseInt(value[i].value) && parseInt(value[i].value) != 0 && isNumber) {
						...
						$('#payment').hide("fast");
						$('#cart').hide("fast");
						$.post('http://esx_rus_shop/buy', JSON.stringify({Count : value[i].value, Item : value[i].id}));
					} 
					else {
						$.post('http://esx_rus_shop/notify', JSON.stringify({msg : "~r~unul din iteme ori nu mai este pe stock ori este cantitatea prea mare."}));
					}
				}
			});
			...
			$("body").on("click", "#bossactions", function() {
				$.post('http://esx_rus_shop/bossactions', JSON.stringify({}));
				$.post('http://esx_rus_shop/escape', JSON.stringify({}));
				location.reload(true);
				$.post('http://esx_rus_shop/emptycart', JSON.stringify({}));
				$('#wrapper').hide("fast");
				$('#payment').hide("fast");
				$('#cart').hide("fast");
			});
